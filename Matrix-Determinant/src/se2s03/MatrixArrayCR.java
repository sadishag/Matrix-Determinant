...
public class MatrixArrayCR {
    ...
    public MatrixArrayCR(long[] matrixArray, int l) {
        ...
        if (Math.sqrt(matrixArray.length) % 1 == 0) {
            ...
            for (int i = 0; i<n; i++) {
                for (int j = 0; j<n; j++) {
                    matArray[j][i] = matrixArray[(j*n) + i];
                }
            }
        } else {
    ...
    public MatrixArrayCR(long[] matrixArray) {
        n = (int) Math.sqrt(matrixArray.length);
    ...
    public MatrixArrayCR convert2DTo1D (long [][] array) {
        ...
        return new MatrixArrayCR(newArray);
    }
    
    
    public MatrixArrayCR getWithoutRowCol(int row, int col) {
    ...
    public long determinant() {
        ...
        for (int r = 0; r<n; r++) {
                MatrixArrayCR newMatrix = getWithoutRowCol(r,0);
